...
void QuadTree::updateParticles(double step, double *localAccX, double *localAccY, std::vector<Particle*> *particles) { 
    // Stockage des accélérations locales (chaque rang calcule sa contribution)

    // We compute the forces exerted on the particles and save them in localAccX and localAccY
    #pragma omp parallel for
    for (size_t i = 0; i < particles->size(); ++i) {
        Particle* particle = (*particles)[i];
        localAccX[i] = 0.0;
        localAccY[i] = 0.0;
        calculateForce(particle, localAccX[i], localAccY[i]);
    }
