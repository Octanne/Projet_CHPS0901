...
int main(int argc, char** argv) {
    ...
    if (rankMPI == 0) std::cout << "Barnes-Hut MPI/OpenMP with recycle memory implementation" << std::endl;
    ...
    std::vector<Particle*> particles; // vector of particles globally
    std::vector<Particle*> particlesLocal; // vector of particles for the current rank
    ...
    QuadTree qt = initQuadTree(particles, windowSizeG, particlesLocal);
...
void launchSimulation(QuadTree &qt, std::vector<Particle *> &particles, Visualizer *qtVisu, double nbSteps, double timeStep, double refreshRate, bool shouldGUI)
{
    ...
    if (!shouldGUI) {
        while (!qtVisu->hasToClose() && (nbSteps == 0 || step < nbSteps))
        {
            ...
            qt.updateParticles(timeStep, localAccX.data(), localAccY.data(), &particles);
            // We update the tree
            qt.buildTree();
            step++;
        }
...
QuadTree initQuadTree(std::vector<Particle *> &particles, double windowSizeG, std::vector<Particle *> &particlesLocal)
{
    QuadTree::setDebugModePtr(Visualizer::ptrDebugMode());

    // We divide the particles list between the MPI processes
    int particlesPerProcess = particles.size() / sizeMPI;
    int start = rankMPI * particlesPerProcess;
    int end = (rankMPI == sizeMPI - 1) ? particles.size() : (rankMPI + 1) * particlesPerProcess;
    for (int i = start; i < end; i++) {
        particlesLocal.push_back(particles[i]);
    }
    QuadTree qt(windowSizeG, 0, 0, &particlesLocal);
    qt.buildTree();
    if (rankMPI == 0) std::cout << "The local quadtrees has been built" << std::endl;
    return qt;
}
